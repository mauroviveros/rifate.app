...
export default defineAction({
  ...
  handler: async (input, { cookies, request }) => {
    ...
    if (buyerError || !buyer) {
      ...
      throw new ActionError({ code: "INTERNAL_SERVER_ERROR", message: "Error al crear el comprador" });
    }
    ...
    if (numbersError) {
      ...
      throw new ActionError({ code: "INTERNAL_SERVER_ERROR", message: "Error al asignar los números" });
    }
